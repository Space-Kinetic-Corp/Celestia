...
template<typename T>
bool
operator<(const Sample<T> &a, const Sample<T> &b)
{
    return a.t < b.t;
}
...
template<typename T>
bool
operator<(const SampleXYZV<T> &a, const SampleXYZV<T> &b)
{
    return a.t < b.t;
}
...
SampledOrbit<T>::SampledOrbit(TrajectoryInterpolation _interpolation) :
    boundingRadius(0.0), period(1.0), lastSample(0), interpolation(_interpolation)
{
}

template<typename T>
void
SampledOrbit<T>::addSample(double t, const Eigen::Matrix<T, 3, 1> &position)
{
...
SampledOrbit<T>::computePosition(double jd) const
{
    ...
    else
    {
        ...
        if (n < 1 || n >= (int)samples.size() || jd < samples[n - 1].t || jd > samples[n].t)
        {
            ...
            n         = iter == samples.end() ? samples.size() : iter - samples.begin();
...
template<typename T>
Eigen::Vector3d
SampledOrbit<T>::computePositionLinear(double jd, int n) const
{
    ...
    return Eigen::Vector3d(
        celmath::lerp(t, (double)s0.position.x(), (double)s1.position.x()),
...
template<typename T>
Eigen::Vector3d
SampledOrbit<T>::computePositionCubic(double jd, int n) const
{
    Sample<T> s0 = n > 1 ? samples[n - 2] : samples[n - 1];
    Sample<T> s1 = samples[n - 1];
    Sample<T> s2 = samples[n];
    Sample<T> s3 = n < (int)samples.size() - 1 ? samples[n + 1] : samples[n];
    ...
    Eigen::Vector3d v0 = n > 1 ? (v10 * (0.5 / (s1.t - s0.t)) + v21 * (0.5 * ih)) * h : v21;
...
template<typename T>
Eigen::Vector3d
SampledOrbit<T>::computeVelocityLinear(double jd, int n) const
{
    Sample<T> s0 = samples[n - 1];
    Sample<T> s1 = samples[n];
...
template<typename T>
Eigen::Vector3d
SampledOrbit<T>::computeVelocityCubic(double jd, int n) const
{
    Sample<T> s0 = n > 1 ? samples[n - 2] : samples[n - 1];
    Sample<T> s1 = samples[n - 1];
    Sample<T> s2 = samples[n];
...
template<typename T>
void
SampledOrbit<T>::sample(double /* startTime */, double /* endTime */, OrbitSampleProc &proc) const
{
...
template<typename T>
void
SampledOrbitXYZV<T>::sample(double /* startTime */, double /* endTime */, OrbitSampleProc &proc)
    const
{
...
template<typename T>
std::unique_ptr<SampledOrbit<T>>
LoadSampledOrbit(
    const fs::path         &filename,
    TrajectoryInterpolation interpolation,
    const fs::path         &colorFilename,
    T /*unused*/)
{
    ...
    if (!colorFilename.empty())
    {
        ...
        if (!SkipComments(inColor))
            return nullptr;

        lastSampleTime = -std::numeric_limits<double>::infinity();
        while (inColor.good())
        {
            double tdb, r, g, b, a;
            inColor >> tdb;
...
ParseXYZVBinaryHeader(std::istream &in, const fs::path &filename)
{
    ...
    if (std::string_view(header.data() + offsetof(XYZVBinaryHeader, magic), XYZV_MAGIC.size())
        != XYZV_MAGIC)
    {
    ...
    std::memcpy(
        &byteOrder,
        header.data() + offsetof(XYZVBinaryHeader, byteOrder),
        sizeof(byteOrder));
    if (byteOrder != static_cast<decltype(byteOrder)>(celestia::compat::endian::native))
    {
        GetLogger()->error(
            _("Unsupported byte order {}, expected {}.\n"),
            byteOrder,
            static_cast<int>(celestia::compat::endian::native));
        return false;
    }
    ...
    {
        GetLogger()->error(
            _("Unsupported digits number {}, expected {}.\n"),
            digits,
...
LoadSampledOrbitXYZVBinary(const fs::path &filename, TrajectoryInterpolation interpolation)
{
    ...
    while (in.good())
    {
        ...
        std::memcpy(
            position.data(),
            data.data() + offsetof(XYZVBinaryData, position),
            sizeof(double) * 3);
        std::memcpy(
            velocity.data(),
            data.data() + offsetof(XYZVBinaryData, velocity),
            sizeof(double) * 3);
...
LoadSampledTrajectorySinglePrec(
    const fs::path         &filename,
    const fs::path         &colorFilename,
    TrajectoryInterpolation interpolation)
{
    return LoadSampledOrbit(filename, interpolation, colorFilename, 0.0f);
}

/*! Load a trajectory file containing double precision positions.
 */
std::unique_ptr<Orbit>
LoadSampledTrajectoryDoublePrec(
    const fs::path         &filename,
    const fs::path         &colorFilename,
    TrajectoryInterpolation interpolation)
{
    return LoadSampledOrbit(filename, interpolation, colorFilename, 0.0);
}
