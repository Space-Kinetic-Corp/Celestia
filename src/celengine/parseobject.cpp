...
CreateKeplerianOrbit(const Hash *orbitData, bool usePlanetUnits)
{
    ...
    if (auto semiMajorAxisValue = orbitData->getLength<double>("SemiMajorAxis", 1.0, distanceScale);
        semiMajorAxisValue.has_value())
    {
        elements.semimajorAxis = *semiMajorAxisValue;
    }
    else if (auto pericenter
             = orbitData->getLength<double>("PericenterDistance", 1.0, distanceScale);
             pericenter.has_value())
    {
    ...
    if (auto periodValue = orbitData->getTime<double>("Period", 1.0, timeScale);
        periodValue.has_value())
    {
    ...
    else if (auto meanLongitude = orbitData->getAngle<double>("MeanLongitude");
             meanLongitude.has_value())
        elements.meanAnomaly
            = *meanLongitude - (elements.argPericenter + elements.longAscendingNode);
...
CreateSampledTrajectory(const Hash *trajData, const fs::path &path)
{
    ...
    // color source
    std::string colorSourceName;
    auto        colorSourceNamePtr     = trajData->getString("ColorSource");
    auto        colorSourceNameDefault = "";
    if (colorSourceNamePtr == nullptr)
    {
        colorSourceName = colorSourceNameDefault;
    }
    else
    {
        colorSourceName = *colorSourceNamePtr;
    }
    ...
    if (const std::string *interpolationString = trajData->getString("Interpolation");
        interpolationString != nullptr)
    {
    ...
    TrajectoryPrecision precision
        = useDoublePrecision ? TrajectoryPrecision::Double : TrajectoryPrecision::Single;

    GetLogger()->verbose("Attempting to load sampled trajectory from source '{}'\n", *sourceName);
    ResourceHandle orbitHandle = GetTrajectoryManager()->getHandle(
        TrajectoryInfo(*sourceName, colorSourceName, path, interpolation, precision));
...
CreateFixedPosition(const Hash *trajData, const Selection &centralObject, bool usePlanetUnits)
{
    ...
    if (auto rectangular = trajData->getLengthVector<double>("Rectangular", 1.0, distanceScale);
        rectangular.has_value())
    {
        // Convert to Celestia's coordinate system
        position = Vector3d(rectangular->x(), rectangular->z(), -rectangular->y());
    }
    else if (auto planetographic = trajData->getSphericalTuple("Planetographic");
             planetographic.has_value())
    {
        if (centralObject.getType() != SelectionType::Body)
        {
            GetLogger()->error(
                "FixedPosition planetographic coordinates are not valid for stars.\n");
            return nullptr;
        }
        ...
        position = centralObject.body()->planetocentricToCartesian(
            180.0 + planetographic->x(),
            planetographic->y(),
            planetographic->z());
    }
    else if (auto planetocentric = trajData->getSphericalTuple("Planetocentric");
             planetocentric.has_value())
    {
        if (centralObject.getType() != SelectionType::Body)
        {
            GetLogger()->error(
                "FixedPosition planetocentric coordinates aren't valid for stars.\n");
            return nullptr;
        }

        // TODO: Change planetocentricToCartesian so that 180 degree offset isn't required
        position = centralObject.body()->planetocentricToCartesian(
            180.0 + planetocentric->x(),
            planetocentric->y(),
            planetocentric->z());
    }
    else
 ...
 */
static bool
ParseStringList(const Hash *table, const string &propertyName, list<string> &stringList)
{
    ...
    if (const ValueArray *array = v->getArray(); array != nullptr)
    {
        ...
        if (std::any_of(
                array->begin(),
                array->end(),
                [](const Value &val) { return val.getType() != ValueType::StringType; }))
            return false;

        // Add strings to stringList
        for (const auto &val : *array) stringList.emplace_back(*val.getString());
...
CreateSpiceOrbit(const Hash *orbitData, const fs::path &path, bool usePlanetUnits)
{
    ...
    if (orbitData->getValue("Kernel") != nullptr)
    {
        ...
        {
            GetLogger()->error(
                "Kernel list for SPICE orbit is neither a string nor array of strings\n");
            return nullptr;
        }
    }
    ...
    if (auto bounding = orbitData->getLength<double>("BoundingRadius", 1.0, distanceScale);
        bounding.has_value())
    {
    ...
    if (beginningDate != nullptr && endingDate != nullptr)
    {
        ...
        orbit = std::make_unique<celestia::ephem::SpiceOrbit>(
            *targetBodyName,
            *originName,
            period,
            boundingRadius,
...
CreateSpiceRotation(const Hash *rotationData, const fs::path &path)
{
    ...
    if (rotationData->getValue("Kernel") != nullptr)
    {
        ...
        {
            GetLogger()->warn(
                "Kernel list for SPICE rotation is neither a string nor array of strings\n");
            return nullptr;
        }
    }
    ...
    // be considered aperiodic.
    auto period
        = rotationData->getTime<double>("Period", 1.0, 1.0 / astro::HOURS_PER_DAY).value_or(0.0);
    ...
    if (beginningDate != nullptr && endingDate != nullptr)
    {
        ...
        rotation = std::make_unique<celestia::ephem::SpiceRotation>(
            *frameName,
            baseFrameName,
            period,
            beginningTDBJD,
    ...
    else
    {
        ...
        rotation
            = std::make_unique<celestia::ephem::SpiceRotation>(*frameName, baseFrameName, period);
    }
...
static celestia::ephem::Orbit *
CreateScriptedOrbit(const Hash *orbitData, const fs::path &path)
{
#if !defined(CELX)
...
CreateOrbit(
    const Selection &centralObject,
    const Hash      *planetData,
    const fs::path  &path,
    bool             usePlanetUnits)
{
    if (const std::string *customOrbitName = planetData->getString("CustomOrbit");
        customOrbitName != nullptr)
    {
...
#ifdef USE_SPICE
    if (const Value *spiceOrbitDataValue = planetData->getValue("SpiceOrbit");
        spiceOrbitDataValue != nullptr)
    {
    ...
    if (const Value *scriptedOrbitValue = planetData->getValue("ScriptedOrbit");
        scriptedOrbitValue != nullptr)
    {
    ...
    if (const Value *sampledTrajDataValue = planetData->getValue("SampledTrajectory");
        sampledTrajDataValue != nullptr)
    {
    ...
    // single precision.
    if (const std::string *sampOrbitFile = planetData->getString("SampledOrbit");
        sampOrbitFile != nullptr)
    {
        GetLogger()->verbose("Attempting to load sampled orbit file '{}'\n", *sampOrbitFile);
        ResourceHandle orbitHandle = GetTrajectoryManager()->getHandle(TrajectoryInfo(
            *sampOrbitFile,
            "",
            path,
    ...
    if (const Value *orbitDataValue = planetData->getValue("EllipticalOrbit");
        orbitDataValue != nullptr)
    {
    ...
    if (const Value *fixedPositionValue = planetData->getValue("FixedPosition");
        fixedPositionValue != nullptr)
    {
        double distanceScale;
        GetDefaultUnits(usePlanetUnits, distanceScale);

        if (auto fixed = planetData->getLengthVector<double>("FixedPosition", 1.0, distanceScale);
            fixed.has_value())
        {
            // Convert to Celestia's coordinate system
...
#else
            Vector3d pos
                = centralBody->planetocentricToCartesian(longlat->x(), longlat->y(), longlat->z());
#endif
...
CreateFixedRotationModel(double offset, double inclination, double ascendingNode)
{
    Quaterniond q = YRotation(-celestia::numbers::pi - offset) * XRotation(-inclination)
...
CreateUniformRotationModel(const Hash *rotationData, double syncRotationPeriod)
{
    // Default to synchronous rotation
    auto period = rotationData->getTime<double>("Period", 1.0, 1.0 / astro::HOURS_PER_DAY)
                      .value_or(syncRotationPeriod);
    ...
    {
        return std::make_unique<celestia::ephem::UniformRotationModel>(
            period,
...
CreateFixedAttitudeRotationModel(const Hash *rotationData)
{
    ...
    Quaterniond q
        = YRotation(-celestia::numbers::pi - heading) * XRotation(-tilt) * ZRotation(-roll);

    return std::make_unique<celestia::ephem::ConstantOrientation>(q);
}

static std::unique_ptr<celestia::ephem::RotationModel>
CreatePrecessingRotationModel(const Hash *rotationData, double syncRotationPeriod)
{
    // Default to synchronous rotation
    double period = rotationData->getTime<double>("Period", 1.0, 1.0 / astro::HOURS_PER_DAY)
                        .value_or(syncRotationPeriod);
    ...
    auto precessionPeriod
        = rotationData->getTime<double>("PrecessionPeriod", 1.0, astro::DAYS_PER_YEAR)
              .value_or(0.0);
    ...
    {
        return std::make_unique<celestia::ephem::PrecessingRotationModel>(
            period,
            static_cast<float>(offset),
            epoch,
            static_cast<float>(inclination),
...
static std::unique_ptr<celestia::ephem::RotationModel>
CreateScriptedRotation(const Hash *rotationData, const fs::path &path)
{
#if !defined(CELX)
...
CreateRotationModel(const Hash *planetData, const fs::path &path, double syncRotationPeriod)
{
    ...
    if (const std::string *customRotationModelName = planetData->getString("CustomRotation");
        customRotationModelName != nullptr)
    {
        ...
        GetLogger()->error(
            "Could not find custom rotation model named '{}'\n",
            *customRotationModelName);
    }

#ifdef USE_SPICE
    if (const Value *spiceRotationDataValue = planetData->getValue("SpiceRotation");
        spiceRotationDataValue != nullptr)
    {
        ...
        {
            if (auto rotationModel = CreateSpiceRotation(spiceRotationData, path);
                rotationModel != nullptr)
            {
    ...
    if (const Value *scriptedRotationValue = planetData->getValue("ScriptedRotation");
        scriptedRotationValue != nullptr)
    {
        ...
        if (auto rotationModel = CreateScriptedRotation(scriptedRotationData, path);
            rotationModel != nullptr)
            return rotationModel.release();
    }

    if (const std::string *sampOrientationFile = planetData->getString("SampledOrientation");
        sampOrientationFile != nullptr)
    {
        ...
        if (auto rotationModel = GetRotationModelManager()->find(orientationHandle);
            rotationModel != nullptr)
        {
            return rotationModel;
        }
    ...
    if (const Value *precessingRotationValue = planetData->getValue("PrecessingRotation");
        precessingRotationValue != nullptr)
    {
        ...
        return CreatePrecessingRotationModel(precessingRotationData, syncRotationPeriod).release();
    }

    if (const Value *uniformRotationValue = planetData->getValue("UniformRotation");
        uniformRotationValue != nullptr)
    {
        ...
        return CreateUniformRotationModel(uniformRotationData, syncRotationPeriod).release();
    }

    if (const Value *fixedRotationValue = planetData->getValue("FixedRotation");
        fixedRotationValue != nullptr)
    {
    ...
    if (const Value *fixedAttitudeValue = planetData->getValue("FixedAttitude");
        fixedAttitudeValue != nullptr)
    {
    ...
    if (auto ascendingNodeVal = planetData->getNumber<float>("EquatorAscendingNode");
        ascendingNodeVal.has_value())
    {
        specified     = true;
    ...
    if (auto precessionVal = planetData->getNumber<double>("PrecessionRate");
        precessionVal.has_value())
    {
        specified      = true;
    ...
    if (specified)
    {
        ...
        {
            rm = std::make_unique<celestia::ephem::UniformRotationModel>(
                period,
                offset,
                epoch,
                inclination,
        ...
        {
            rm = std::make_unique<celestia::ephem::PrecessingRotationModel>(
                period,
                offset,
                epoch,
                inclination,
...
CreateDefaultRotationModel(double syncRotationPeriod)
{
    ...
    {
        rotation = std::make_unique<celestia::ephem::UniformRotationModel>(
            syncRotationPeriod,
            0.0f,
            astro::J2000,
            0.0f,
...
{
    if (compareIgnoringCase(label, "x") == 0 || compareIgnoringCase(label, "+x") == 0)
    {
        return 1;
    }

    if (compareIgnoringCase(label, "y") == 0 || compareIgnoringCase(label, "+y") == 0)
    {
        return 2;
    }

    if (compareIgnoringCase(label, "z") == 0 || compareIgnoringCase(label, "+z") == 0)
    {
        return 3;
    }
...
getVectorTarget(const Universe &universe, const Hash *vectorData)
{
    ...
    {
        GetLogger()->warn(
            "Bad two-vector frame: target object '{}' of vector not found.\n",
            *targetName);
        return Selection();
    }
...
getVectorObserver(const Universe &universe, const Hash *vectorData)
{
    ...
    {
        GetLogger()->warn(
            "Bad two-vector frame: observer object '{}' of vector not found.\n",
            obsObject.getName());
        return Selection();
    }
...
static FrameVector *
CreateFrameVector(const Universe &universe, const Selection &center, const Hash *vectorData)
{
    ...
    const Selection &defaultCenter)
{
    ...
    FrameVector *primaryVector   = CreateFrameVector(universe, center, primaryData);
    FrameVector *secondaryVector = CreateFrameVector(universe, center, secondaryData);

    TwoVectorFrame *frame = nullptr;
    if (primaryVector != nullptr && secondaryVector != nullptr)
    {
        frame = new TwoVectorFrame(
            center,
            *primaryVector,
            primaryAxis,
            *secondaryVector,
...
CreateTopocentricFrame(const Selection &center, const Selection &target, const Selection &observer)
{
    ...
    const Selection &defaultObserver)
{
    ...
    if (const std::string *centerName = frameData->getString("Center"); centerName != nullptr)
    {
        ...
        {
            GetLogger()->error(
                "Center object '{}' for topocentric frame not found.\n",
                *centerName);
            return nullptr;
        }
    ...
    else
    {
        ...
        {
            GetLogger()->error(
                "Target object '{}' for topocentric frame not found.\n",
                *targetName);
            return nullptr;
        }
    ...
    else
    {
        ...
        {
            GetLogger()->error(
                "Observer object '{}' for topocentric frame not found.\n",
                *observerName);
            return nullptr;
        }
    }
    ...
    Body            *defaultObserver)
{
    ...
    if (const Value *value = frameData->getValue("Topocentric"); value != nullptr)
    {
        ...
        return CreateTopocentricFrame(
            universe,
            topocentricData,
            defaultCenter,
            Selection(defaultObserver));
    }
