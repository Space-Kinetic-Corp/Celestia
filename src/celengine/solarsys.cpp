...
const int CLASSES_UNCLICKABLE = Body::Invisible | Body::Diffuse;

const int CLASSES_INVISIBLE_AS_POINT
    = Body::Invisible | Body::SurfaceFeature | Body::Component | Body::Diffuse;

const int CLASSES_SECONDARY_ILLUMINATOR = Body::Planet | Body::Moon | Body::MinorMoon
                                          | Body::DwarfPlanet | Body::Asteroid | Body::Comet;

// lookup table generated by gperf (solarsys.gperf)
#include "solarsys.inc"

int
GetClassificationId(std::string_view className)
{
    auto ptr = ClassificationMap::getClassification(className.data(), className.size());
    return ptr == nullptr ? Body::Unknown : ptr->classification;
}
...
bool
isFrameCircular(const ReferenceFrame &frame, ReferenceFrame::FrameType frameType)
{
    return frame.nestingDepth(MaxFrameDepth, frameType) > MaxFrameDepth;
}

std::unique_ptr<Location>
CreateLocation(const Hash *locationData, Body *body)
{
    ...
    if (const std::string *featureTypeName = locationData->getString("Type");
        featureTypeName != nullptr)
...
inline void
SetOrUnset(Dst &dst, Flag flag, bool cond)
{
    if (cond)
        dst |= flag;
    else
...
void
FillinSurface(const Hash *surfaceData, Surface *surface, const fs::path &path)
{
    ...
    if (auto specularPower = surfaceData->getNumber<float>("SpecularPower");
        specularPower.has_value())
    ...
    unsigned int bumpFlags
        = TextureInfo::WrapTexture | TextureInfo::AllowSplitting | TextureInfo::LinearColorspace;
    ...
    SetOrUnset(
        surface->appearanceFlags,
        Surface::ApplyBumpMap,
        (bumpTexture != nullptr || normalTexture != nullptr));
    ...
    SetOrUnset(
        surface->appearanceFlags,
        Surface::SpecularReflection,
        surface->specularColor != Color(0.0f, 0.0f, 0.0f));
...
Selection
GetParentObject(PlanetarySystem *system)
{
    Selection parent;
    ...
    double                                previousPhaseEnd)
{
    ...
    {
        GetLogger()->error(
            "Error: Beginning can only be specified for initial phase of timeline.\n");
        return nullptr;
    }
    ...
    {
        GetLogger()->error(
            "Error: Ending is required for all timeline phases other than the final one.\n");
        return nullptr;
    }
    ...
    {
        orbitFrame
            = CreateReferenceFrame(universe, frameValue, defaultOrbitFrame->getCenter(), body);
    ...
    {
        bodyFrame
            = CreateReferenceFrame(universe, bodyFrameValue, defaultBodyFrame->getCenter(), body);
    ...
    celestia::ephem::Orbit *orbit
        = CreateOrbit(orbitFrame->getCenter(), phaseData, path, usePlanetUnits);
    if (!orbit)
    {
    ...
    celestia::ephem::RotationModel *rotationModel
        = CreateRotationModel(phaseData, path, orbit->getPeriod());
    if (!rotationModel)
    {
    ...
    const ReferenceFrame::SharedConstPtr &defaultBodyFrame)
{
    ...
    {
        GetLogger()->error(
            "Error in timeline of '{}': timeline array is empty.\n",
            body->getName());
        return nullptr;
    }
    ...
    for (auto iter = timelineArray->begin(); iter != timelineArray->end(); iter++)
    {
        ...
        {
            GetLogger()->error(
                "Error in timeline of '{}': phase {} is not a property group.\n",
                body->getName(),
        ...
        auto phase = CreateTimelinePhase(
            body,
            universe,
            phaseData,
            path,
    ...
    BodyType         bodyType)
{
    ...
    if (value != nullptr)
    {
        ...
        std::unique_ptr<Timeline> timeline = CreateTimelineFromArray(
            body,
            universe,
            timelineArray,
            path,
    ...
    celestia::ephem::Orbit *newOrbit
        = CreateOrbit(orbitFrame->getCenter(), planetData, path, !orbitsPlanet);
    if (newOrbit == nullptr && orbit == nullptr)
    {
        ...
        {
            GetLogger()->error(
                "No valid orbit specified for object '{}'. Skipping.\n",
                body->getName());
            return false;
        }
    }
    ...
    celestia::ephem::RotationModel *newRotationModel
        = CreateRotationModel(planetData, path, syncRotationPeriod);
...
void
ReadAtmosphere(Body *body, const Hash *atmosData, const fs::path &path, DataDisposition disposition)
{
    ...
    if (auto miePhaseAsymmetry = atmosData->getNumber<float>("MieAsymmetry");
        miePhaseAsymmetry.has_value())
    ...
    if (auto absorptionCoeff = atmosData->getVector3<float>("Absorption");
        absorptionCoeff.has_value())
    ...
    {
        atmosphere->cloudTexture.setTexture(*cloudTexture, path, TextureInfo::WrapTexture);
    }

    if (const std::string *cloudNormalMap = atmosData->getString("CloudNormalMap");
        cloudNormalMap != nullptr)
    {
        atmosphere->cloudNormalMap.setTexture(
            *cloudNormalMap,
            path,
            TextureInfo::WrapTexture | TextureInfo::LinearColorspace);
    }

    if (auto cloudShadowDepth = atmosData->getNumber<float>("CloudShadowDepth");
        cloudShadowDepth.has_value())
    {
...
void
ReadRings(Body *body, const Hash *ringsData, const fs::path &path, DataDisposition disposition)
{
    ...
    BodyType           bodyType)
{
    ...
    if (const std::string *classificationName = planetData->getString("Class");
        classificationName != nullptr)
    ...
    if (const std::string *infoURL = planetData->getString("InfoURL"); infoURL != nullptr)
    {
        ...
        if (infoURL->find(':') == std::string::npos && !path.empty())
        {
            ...
            modifiedURL = p.size() > 1 && p[1] == ':' ? fmt::format("file:///{}/{}", p, *infoURL) :
    ...
    if (auto albedo = planetData->getNumber<float>("Albedo"); albedo.has_value())
    {
        // TODO: make this warn
        GetLogger()->verbose(
            "Deprecated parameter Albedo used in {} definition.\nUse GeomAlbedo & BondAlbedo "
            "instead.\n",
            name);
    ...
    if (auto temperature = planetData->getNumber<float>("Temperature");
        temperature.has_value() && *temperature > 0.0f)
        body->setTemperature(*temperature);
    if (auto tempDiscrepancy = planetData->getNumber<float>("TempDiscrepancy");
        tempDiscrepancy.has_value())
    ...
    {
        auto geometryCenter
            = planetData->getVector3<float>("MeshCenter").value_or(Eigen::Vector3f::Zero());
        ...
        ResourceHandle geometryHandle = GetGeometryManager()->getHandle(
            GeometryInfo(*geometry, path, geometryCenter, 1.0f, isNormalized));
    ...
    // read the sensor
    if (auto sensorgeo = planetData->getValue("Sensor"); sensorgeo != nullptr)
    {
        auto sensorData = sensorgeo->getHash();

        if (sensorData)
        {
            auto *sensor = new SensorGeometry();
            sensor->setObserver(body);

            std::string targetName;
            if (sensorData->getString("Target"))
            {
                Body *target = universe.findPath("Target", {}).body();
                if (target)
                {
                ...
                {
                    GetLogger()->error(_("Can't find target for sensor.\n"));
                }
            }
            else
            {
                GetLogger()->error(_("No target specified for sensor.\n"));
            }

            double range = 1.0;
            if (auto rangeOk = sensorData->getNumber<double>("Range"); rangeOk.has_value())
                sensor->setRange(range);

            double horizontalFov = celmath::degToRad(5.0);
            double verticalFov   = celmath::degToRad(5.0);
            auto   horFovOk      = sensorData->getNumber<double>("HorizontalFOV");
            auto   verFovOk      = sensorData->getNumber<double>("VerticalFOV");
            if (horFovOk.has_value() && verFovOk.has_value())
                sensor->setFOVs(celmath::degToRad(horizontalFov), celmath::degToRad(verticalFov));

            double minAzimuth   = -20.0;
            double maxAzimuth   = 20.0;
            double minElevation = 5.0;
            double maxElevation = 10.0;
            auto   minAzOk      = sensorData->getNumber<double>("MinAzimuth");
            auto   maxAzOk      = sensorData->getNumber<double>("MaxAzimuth");
            auto   minElOk      = sensorData->getNumber<double>("MinElevation");
            auto   maxElOk      = sensorData->getNumber<double>("MaxElevation");

            if (minAzOk.has_value() && maxAzOk.has_value() && minElOk.has_value()
                && maxElOk.has_value())
                sensor->setAzimuthElevation(
                    celmath::degToRad(minAzimuth),
                    celmath::degToRad(maxAzimuth),
                    celmath::degToRad(minElevation),
                    celmath::degToRad(maxElevation));
            ...
            if (auto frustColorOk = sensorData->getColor("FrustumColor"); frustColorOk.has_value())
                sensor->setFrustumColor(frustumColor);

            if (auto frustBaseColorOk = sensorData->getColor("FrustumBaseColor");
                frustBaseColorOk.has_value())
                sensor->setFrustumBaseColor(frustumBaseColor);

            if (auto frustOpacityOk = sensorData->getNumber<float>("FrustumOpacity");
                frustOpacityOk.has_value())
                sensor->setFrustumOpacity(frustumOpacity);

            if (auto gridOpacityOk = sensorData->getNumber<float>("GridOpacity");
                gridOpacityOk.has_value())
                ;

            float       traceDuration = -1.0f; // Disabled by default
            float       traceOpacity  = 0.60f;
            Color       traceColor(frustumBaseColor);
            std::string traceColorFile;
            if (auto traceDurOk = sensorData->getNumber<float>("TraceDuration");
                traceDurOk.has_value())
                sensor->setTraceDuration(traceDuration);

            if (auto traceOpacityOk = sensorData->getNumber<float>("TraceOpacity");
                traceOpacityOk.has_value())
                sensor->setTraceOpacity(traceOpacity);
            sensorData->getColor("TraceColor");

            if (auto traceColorOk = sensorData->getColor("TraceColor"); traceColorOk.has_value())
                sensor->setTraceColor(traceColor);

            if (auto traceColorFileOk = sensorData->getString("TraceColorFile"))
            {
                // Easy way to retrieve the path...
                traceColorFile = path.string() + "/data/" + traceColorFile;
                sensor->setTraceColorFile(traceColorFile);
            }

            std::string shape = "elliptical";
            sensorData->getString("Shape");
            if (compareIgnoringCase(shape, "elliptical") == 0)
            {
                sensor->setShape(SensorGeometry::EllipticalShape);
            }
            ...
            else if (compareIgnoringCase(shape, "azimuthelevation") == 0)
            {
                sensor->setShape(SensorGeometry::AzimuthElevationShape);
            }

            std::string  resName = std::string("sensor") + targetName + Selection(body).getName();
            GeometryInfo info(resName, path, Eigen::Vector3f::Zero(), 1.0f, false);
            info.resource = dynamic_cast<Geometry *>(sensor);
    ...
    // read the catalog
    if (body->getClassification() == Body::Cluster)
    {
        if (auto catalog = planetData->getValue("Catalog"); !catalog->isNull())
        {
            if (catalog && catalog->getType() == ValueType::HashType)
            {
                auto catalogHash       = catalog->getHash();
                auto populationCatalog = catalogHash->getString("Population");

                auto populationSymbol = catalogHash->getString("Symbol");

                //                uint32_t symbolSize = 10;
                auto symbolSize = catalogHash->getNumber<uint32_t>("SymbolSize");

                //                double objectRadius = 0.001;
                auto objectRadius = catalogHash->getNumber<double>("ObjectRadius");

                if (!populationCatalog->empty())
                {
                    GeometryInfo info(*populationCatalog, path, Vector3f::Zero(), 1.0f, false);
...
//                    auto popstr = *populationCatalog;
                    ...
                    //                    body->addReferenceMark(new Cluster(
                    //                        universe,
                    //                        *body,
                    //                        path.string(),
                    //                        *populationCatalog,
                    ...
                    body->addReferenceMark(std::make_unique<Cluster>(
                        universe,
                        *body,
                        path.string(),
                        *populationCatalog,
...
bool
LoadSolarSystemObjects(std::istream &in, Universe &universe, const fs::path &directory)
{
        ...
        if (bodyType != UnknownBodyType)
        {
            ...
            else
            {
                sscError(
                    tokenizer,
                    fmt::sprintf(
                        _("parent body '%s' of '%s' not found.\n"),
                        parentName,
                        primaryName));
            }
            ...
            if (parentSystem != nullptr)
            {
                ...
                if (existingBody)
                {
                    if (disposition == DataDisposition::Add)
                        sscError(
                            tokenizer,
                            fmt::sprintf(
                                _("warning duplicate definition of %s %s\n"),
                                parentName,
                                primaryName));
                ...
                if (body != nullptr)
                {
                    UserCategory::loadCategories(
                        body,
                        *objectData,
                        disposition,
                        directory.string());
                    if (disposition == DataDisposition::Add)
                        for (const auto &name : names) body->addAlias(name);
                }
            }
        }
