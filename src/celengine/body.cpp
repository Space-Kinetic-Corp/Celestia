...
Body::Body(PlanetarySystem *_system, const std::string &_name) :
    system(_system), orbitVisibility(UseClassVisibility)
{
    setName(_name);
 ...
 */
void
Body::setName(const string &name)
{
    names[0]             = name;
 ...
 */
void
Body::addAlias(const string &alias)
{
...
Body::getTemperature(double time) const
{
    ...
    if (sun->getVisibility()) // the sun is a star
    {
        ...
        temp              = sun->getTemperature() * pow(1.0f - getBondAlbedo(), 0.25f)
    ...
    else // the sun is a barycenter
    {
        ...
        temp = std::pow((1.0f - getBondAlbedo()) * flux, 0.25f)
               * (celestia::numbers::sqrt2_v<float> * 0.5f);
    }
...
void
Body::setGeometryOrientation(const Quaternionf &orientation)
{
 ...
 */
void
Body::setSemiAxes(const Vector3f &_semiAxes)
{
    semiAxes = _semiAxes;
...
// VTS //
void
Body::setMagCoeff(float coeff)
{
    timeline->setMagCoeff(coeff);
}
...
Body::isSphere() const
{
    return (geometry == InvalidResource) && (semiAxes.x() == semiAxes.y())
...
void
Body::setSatellites(std::unique_ptr<PlanetarySystem> &&ssys)
{
    satellites = std::move(ssys);
}
...
void
Body::setRings(std::unique_ptr<RingSystem> &&_rings)
{
...
void
Body::scaleRings(float scaleFactor)
{
    if (rings == nullptr)
        return;
...
void
Body::setAtmosphere(std::unique_ptr<Atmosphere> &&_atmosphere)
{
...
Body::getVelocity(double tdb) const
{
    ...
    {
        Vector3d r = Selection(const_cast<Body *>(this))
                         .getPosition(tdb)
                         .offsetFromKm(orbitFrame->getCenter().getPosition(tdb));
...
Body::getLocalToAstrocentric(double tdb) const
{
    const TimelinePhase *phase = timeline->findPhase(tdb).get();
    Vector3d             p
        = phase->orbitFrame()->convertToAstrocentric(phase->orbit()->positionAtTime(tdb), tdb);
 ...
 */
Quaterniond
Body::getEclipticToBodyFixed(double tdb) const
{
    const TimelinePhase *phase = timeline->findPhase(tdb).get();
    return phase->rotationModel()->orientationAtTime(tdb) * phase->bodyFrame()->getOrientation(tdb);
}
...
Vector3d
Body::planetocentricToCartesian(const Vector3d &lonLatAlt) const
{
...
Body::geodeticToCartesian(double lon, double lat, double alt) const
{
    ...
    double v = semiAxes.x()
               / std::sqrt(1.0 - e2x * square(sinphi) - e2e * square(cosphi) * square(sintheta));
...
Vector3d
Body::geodeticToCartesian(const Vector3d &lonLatAlt) const
{
 ...
 */
Vector3d
Body::eclipticToPlanetocentric(const Vector3d &ecl, double tdb) const
{
...
float
Body::getLuminosity(const Star &sun, float distanceFromSun) const
{
    return getLuminosity(sun.getLuminosity(), distanceFromSun);
}

float
Body::getLuminosity(float sunLuminosity, float distanceFromSun) const
{
 ...
 */
float
Body::getApparentMagnitude(const Star &sun, float distanceFromSun, float distanceFromViewer) const
{
    return astro::lumToAppMag(
        getLuminosity(sun, distanceFromSun),
 ...
 */
float
Body::getApparentMagnitude(float sunLuminosity, float distanceFromSun, float distanceFromViewer)
    const
{
    return astro::lumToAppMag(
        getLuminosity(sunLuminosity, distanceFromSun),
        astro::kilometersToLightYears(distanceFromViewer));
}
    ...
    const Vector3d &viewerPosition) const
{
    ...
    float  illuminatedFraction
        = (float)(1.0 + (viewerPosition / distanceToViewer).dot(sunPosition / distanceToSun))
          / 2.0f;

    return astro::lumToAppMag(
        getLuminosity(sunLuminosity, (float)distanceToSun) * illuminatedFraction,
...
void
Body::setClassification(int _classification)
{
...
void
Body::setInfoURL(const string &_infoURL)
{
    infoURL = _infoURL;
}

Surface *
Body::getAlternateSurface(const string &name) const
{
    if (!altSurfaces)
        return nullptr;
...
void
Body::addAlternateSurface(const string &name, std::unique_ptr<Surface> &&altSurface)
{
    if (!altSurfaces)
...
void
Body::addLocation(std::unique_ptr<Location> &&loc)
{
    if (!loc)
        return;
...
Location *
Body::findLocation(std::string_view name, bool i18n) const
{
    if (!locations)
        return nullptr;

    auto iter = i18n ? std::find_if(
                    locations->cbegin(),
                    locations->cend(),
                    [&name](const auto &loc)
                    {
                        return UTF8StringCompare(name, loc->getName(false)) == 0
...
Body::findReferenceMark(const string &tag) const
{
    ...
    auto iter = std::find_if(
        ...
        referenceMarks->end(),
        [&tag](const auto &rm) { return rm->getTag() == tag; });
    return iter == referenceMarks->end() ? nullptr : iter->get();
}

/*! Find the first reference mark with the specified tag mutable. If the body has
 *  no reference marks with the specified tag, this method will return
 *  nullptr.
 */
ReferenceMark *
Body::findReferenceMarkMut(const std::string &tag) const
{
    if (referenceMarks == nullptr)
        return nullptr;
...
// VTS //
/*! Sets whether or not the object label is visible (added for VTS)
 */
void
Body::setLabelVisible(bool _visible)
{
    labelvisible = _visible ? 1 : 0;
}

/*! Sets whether or not the object hierarchy is visible. (added for VTS)
 */
void
Body::setHierarchyVisible(bool _visible)
{
    hierarchyvisible = _visible ? 1 : 0;
    ...
    for (int i = 0; i < satellites->getSystemSize(); i++)
    {
        ...
        body->setHierarchyVisible(_visible);
    }
}
 ...
 */
void
Body::setVisibleAsPoint(bool _visibleAsPoint)
{
 ...
 */
void
Body::setOrbitColorOverridden(bool _override)
{
 ...
 */
void
Body::setOrbitColor(const Color &c)
{
    orbitColor = c;
}
 ...
 */
void
Body::setCometTailColor(const Color &c)
{
    cometTailColor = c;
}
 ...
 */
void
Body::setSecondaryIlluminator(bool enable)
{
...
Eigen::Quaterniond
Body::getOrientationCorrection3(double tdb) const
{
    // Is Body a 3ds mesh ?
    if (getGeometry() != InvalidResource)
    {
        if (dynamic_cast<const celestia::ephem::ConstantOrientation *>(getRotationModel(tdb)))
        {
            // Body is oriented by a fixed value in SSC file
            return Quaterniond(AngleAxis<double>(celestia::numbers::pi / 2, Vector3d::UnitX()))
                   * Quaterniond(AngleAxis<double>(celestia::numbers::pi, Vector3d::UnitY()));
        }
        if (dynamic_cast<const celestia::ephem::SampledOrientation *>(getRotationModel(tdb)))
        {
            // Body is oriented by a quaternion file
            return Quaterniond(AngleAxis<double>(celestia::numbers::pi / 2, Vector3d::UnitX()))
                   * Quaterniond(AngleAxis<double>(celestia::numbers::pi, Vector3d::UnitY()));
        }
        // VSOP Orientation
...
PlanetarySystem::PlanetarySystem(Body *_primary) : star(nullptr), primary(_primary)
{
    if (primary && primary->getSystem())
        star = primary->getSystem()->getStar();
}

PlanetarySystem::PlanetarySystem(Star *_star) : star(_star)
{
}
    ...
    bool                      deepSearch) const
{
    ...
    {
        auto satelliteSystem = sat->getSatellites();
        if (satelliteSystem != nullptr)
            satelliteSystem->getCompletion(completion, _name);
    }
}

float Body::getMinPixelSize() const
{
    return minPixelSize;
}
void Body::setMinPixelSize(float value)
{
    minPixelSize = value;
}
