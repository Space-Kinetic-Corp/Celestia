    ...
    Renderer                 *renderer)
{
    ...
    if (double t = 0.0; testIntersection(
                            Eigen::ParametrizedLine<double, 3>(viewRayOrigin, pos - viewRayOrigin),
                            Ellipsoidd(semiAxes.cast<double>()),
                            t)
                        && t >= 1.0)
    {
        // Compute the position of the label
        Eigen::Vector3d labelPos
            = bodyCenter + bodyOrientation.conjugate() * pos * (1.0 + labelOffset);
...
PlanetographicGrid::PlanetographicGrid(const Body &_body) : body(_body)
{
    ...
    const Matrices        &m) const
{
    ...
    Eigen::Affine3f transform
        = Eigen::Translation3f(pos) * qf.conjugate() * Eigen::Scaling(scale * semiAxes);
    ...
    for (float latitude = -90.0f + latitudeStep; latitude < 90.0f; latitude += latitudeStep)
    {
        ...
        Eigen::Matrix4f mvcur
            = modelView * celmath::translate(0.0f, std::sin(phi), 0.0f) * celmath::scale(r);
        if (latitude == 0.0f)
        {
            latitudeRenderer->finish();
            equatorRenderer->render(
                { &projection, &mvcur },
    ...
    for (float longitude = 0.0f; longitude <= 180.0f; longitude += longitudeStep)
    {
        Eigen::Matrix4f mvcur
            = modelView
              * celmath::rotate(Eigen::AngleAxisf(degToRad(longitude), Eigen::Vector3f::UnitY()));

        longitudeRenderer->render(
            { &projection, &mvcur },
            Renderer::PlanetographicGridColor,
            circleSubdivisions + 1);
...
PlanetographicGrid::InitializeGeometry(const Renderer &renderer)
{
    ...
    latitudeRenderer = new LineRenderer(
        renderer,
        1.0f,
        LineRenderer::PrimType::LineStrip,
        LineRenderer::StorageType::Static);
    equatorRenderer = new LineRenderer(
        renderer,
        2.0f,
        LineRenderer::PrimType::LineStrip,
        LineRenderer::StorageType::Static);
    longitudeRenderer = new LineRenderer(
        renderer,
        1.0f,
        LineRenderer::PrimType::LineStrip,
        LineRenderer::StorageType::Static);
