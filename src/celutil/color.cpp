...
buildX11ColorMap()
{
    return new ColorMap{
        ...
        { "lightgoldenrodyellow"sv,
          Color((std::uint8_t)250, (std::uint8_t)250, (std::uint8_t)210) },
...
bool
parseHexColor(std::string_view s, Color &c)
{
    ...
    case 3: // rgb
        ...
        {
            c = Color(
                static_cast<std::uint8_t>((value >> 8) * UINT32_C(0x11)),
 ...
 */
bool
Color::parse(std::string_view s, Color &c)
{
    using celestia::compat::from_chars;
...
{
Color
lighten(const Color &color, float amount)
{
    float r = std::min(color.red() + amount, 1.0f);
    float g = std::min(color.green() + amount, 1.0f);
    float b = std::min(color.blue() + amount, 1.0f);
